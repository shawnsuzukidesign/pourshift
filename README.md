...
**Why we are not advertising founding pricing yet:**

Pre-launch, we have no signal on whether $200 is the right hire fee
for the Bay Area. Publishing a higher "standard" anchor commits us
to a number we haven't validated, and locking 250 venues into a
36-month rate before we've processed a single hire is a real
flexibility cost. Adding a founding promo later ("first 100 venues
this quarter lock in current pricing for 36 months") is easy.
Retracting a publicly advertised one is messy. The CSS hooks
(`.fee-strip.founding`, `.founding-tag`) are kept in styles.css so
the promo can be re-introduced without a rebuild.

**Founding-promo playbook (reactivate when ready):**

When we are ready to raise standard pricing (typical trigger: 4-6
months post-launch with social proof, or when current pricing is
clearly under-priced based on hire-rate data), reactivate as a
time-boxed launch promo:

- Decide the new standard pricing (e.g. $75 invite / $600 hire) and
  decide the founding offer (e.g. lock current $50/$200 for 36 months).
- Decide the cap. Either a venue count (e.g. first 100 sign-ups
  after the promo opens) or a date window (e.g. anyone who signs up
  before <date>). Date windows are easier to communicate and don't
  need a real-time counter.
- Lock period starts at **first paid invite**, not waitlist signup,
  to prevent squatters from burning the window before activating.
- Add `founding_member` boolean and `founding_locked_until` timestamp
  to the venue row (or a venue_billing table). At charge time:

  ```
  if venue.founding_member and now() < venue.founding_locked_until:
      use founding pricing
  else:
      use standard pricing
  ```

- Re-introduce the founding badge on the index.html pricing card by
  re-adding the `founding` class to `.fee-strip` and the
  `.founding-tag` div. CSS is already in place.
- Re-add the founding-member clause to Terms §6, and the founding
  copy to the FAQ "What does it cost?" answer.
- Decide whether to honor every existing waitlist signup at the new
  founding rate (more goodwill, larger locked-rate pool) or only
  new sign-ups during the promo window (better margin protection).
